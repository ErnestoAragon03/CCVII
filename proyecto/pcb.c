...
// Ahora definimos el tope del stack (última dirección válida del espacio reservado)
#define STACK_TOP_P1   (0x80018000 + STACK_SIZE - 4)
#define STACK_TOP_P2   (0x80028000 + STACK_SIZE - 4)
...
int create_process(int pid) {
    ...
    if (pid == 0) {
        stack_top = (unsigned int *)STACK_TOP_P1;
        entry_point = ENTRY_P1;
    } else if (pid == 1) {
        stack_top = (unsigned int *)STACK_TOP_P2;
    ...
    stack_top[13] = entry_point; // LR: a dónde saltar (PC)
    stack_top[14] = 0x10;        // CPSR: modo usuario
